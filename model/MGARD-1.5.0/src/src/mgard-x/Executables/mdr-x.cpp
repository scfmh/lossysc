...
void read_mdr(mgard_x::MDR::RefactoredMetadata &refactored_metadata,
  ...
  std::cout << "length: " <<refactored_metadata.metadata.size() << std::endl;
...
int launch_refactor(mgard_x::DIM D, enum mgard_x::data_type dtype,
                    ...
                    mgard_x::SIZE max_memory_footprint, bool adaptive_resolution,double tol, double s,
  ...
  size_t original_size = 1;
  size_t in_size = 0;
  //if (std::string(input_file).compare("none") != 0 || !config.mdr_adaptive_resolution) {
    if (std::string(input_file).compare("random") == 0) {
 ...
 // mgard_x::pin_memory(compressed_data, original_size * sizeof(T), config);
  ...
  mgard_x::MDR::RefactoredMetadata refactored_metadata_1;
  mgard_x::MDR::RefactoredData refactored_data_1; 
 /* read_mdr(refactored_metadata, refactored_data, output_file);
  ...
  for (auto &metadata : refactored_metadata.metadata) {
   ...
   mgard_x::MDR::MDReconstruct(refactored_metadata, refactored_data,reconstructed_data, config, false, original_data);*/
  
  //mgard_x::MDR::RefactoredMetadata refactored_metadata;
  //mgard_x::MDR::RefactoredData refactored_data;
  read_mdr(refactored_metadata, refactored_data, output_file);
  //std::cout << "this is the total subdomains" << refactored_data.num_subdomains << std::endl;
  std::vector <double> tol_vec{tol, tol/10, tol/100, tol/1000};
  refactored_metadata.InitializeForReconstruction();
  

 for(int error_len =0; error_len < tol_vec.size(); error_len++){
   std::cout << "this is the error " << tol_vec[error_len] << std::endl;
  for (auto &metadata : refactored_metadata.metadata) {
    //metadata.requested_tol = tol;
    metadata.requested_tol = tol_vec[error_len];
    metadata.requested_s = s;
  }
 ...
 mgard_x::MDR::MDRequest(refactored_metadata,config);
  ...
  //config.mdr_adaptive_resolution = !adaptive_resolution;
  mgard_x::MDR::MDReconstruct(refactored_metadata, refactored_data, reconstructed_data, config, false, original_data);
  std::cout << "is initialized or not " <<reconstructed_data.IsInitialized()<<std::endl;
  for (auto &metadata : refactored_metadata.metadata) {
     std::cout << "hhhhh" << std::endl; //requested_level_num_bitplanes
     std::cout << (int)metadata.loaded_level_num_bitplanes[0] <<  " "<<  (int)metadata.loaded_level_num_bitplanes[1] << " " << (int)metadata.loaded_level_num_bitplanes[2] ;
     std::cout << (int)metadata.requested_level_num_bitplanes[0] <<  " "<<  (int)metadata.requested_level_num_bitplanes[1] << " " << (int)metadata.requested_level_num_bitplanes[2] ;
     metadata.DoneLoadingBitplans();
     //metadata.DoneReconstruct();
     std::cout << "hhhhh" << std::endl; //requested_level_num_bitplanes
     std::cout << (int)metadata.loaded_level_num_bitplanes[0] <<  " "<<  (int)metadata.loaded_level_num_bitplanes[1] << " " << (int)metadata.loaded_level_num_bitplanes[2] ;
     std::cout << (int)metadata.requested_level_num_bitplanes[0] <<  " "<<  (int)metadata.requested_level_num_bitplanes[1] << " " << (int)metadata.requested_level_num_bitplanes[2] ;
    }
  ...
  std::cout << "is initialized or not " <<reconstructed_data.IsInitialized()<<std::endl;                                   
  //config.mdr_adaptive_resolution = !adaptive_resolution;
  //reconstructed_data.Initialize(reconstructed_data.num_subdomains);
  mgard_x::MDR::MDReconstruct(refactored_metadata, refactored_data, reconstructed_data, config, false, original_data);
  for (auto &metadata : refactored_metadata.metadata) {
     std::cout << "hhhhh" << std::endl; //requested_level_num_bitplanes
     std::cout << (int)metadata.loaded_level_num_bitplanes[0] <<  " "<<  (int)metadata.loaded_level_num_bitplanes[1] << " " << (int)metadata.loaded_level_num_bitplanes[2] ;
   ...
   int subdomain_id = 0;
  std::cout << " this is the shape " << reconstructed_data.shape[0][0] << std::endl;
  for (int subdomain_id = 0; subdomain_id < reconstructed_data.data.size();
   ...
   //decompressed_data = reconstructed_data.data[0];
  print_statistics<double>(s, mode, shape, (double *)original_data,(double *)reconstructed_data.data[0],tol_vec[error_len],config.normalize_coordinates);
  }
...
bool try_refactoring(int argc, char *argv[]) {
  ...
  mgard_x::error_bound_type mode; // REL or ABS
  ...
  for (int repeat_iter = 0; repeat_iter < repeat; repeat_iter++) {
    if (dtype == mgard_x::data_type::Double) {
      launch_refactor<double>(
          ...
          dev_type, num_dev, verbose, prefetch, max_memory_footprint, false, tol, s, mode);
