...
public:
  ...
  MGARDX_CONT
    ...
    printf("LinearQuantizer block tbz %d, tby %d, tbx %d \n", tbz, tby, tbx);
    //printf(" this is LinearQuantizer queue index %d, total_thread_z %d, total_thread_y %d, total_thread_x %d, tbx %d, tby %d, tbz %d, gridx %d , gridy %d, gridz %d \n",queue_idx, total_thread_z, total_thread_y, total_thread_x, tbx, tby, tbz, gridx, gridy, gridz);
...
public:
  ...
  GenTask(int queue_idx) {
    ...
    gridy = ceil((float)total_thread_y / tby);
    gridx = ceil((float)total_thread_x / tbx);
    printf("Outlier block tbz %d, tby %d, tbx %d \n",  tbz, tby, tbx);
    //printf(" this is Outlier queue index %d, total_thread_z %d, total_thread_y %d, total_thread_x %d, tbx %d, tby %d, tbz %d, gridx %d , gridy %d, gridz %d \n",queue_idx, total_thread_z, total_thread_y, total_thread_x, tbx, tby, tbz, gridx, gridy, gridz);
    return Task(functor, gridz, gridy, gridx, tbz, tby, tbx, sm_size, queue_idx,
                std::string(Name));
  }
  ...
  void Quantize(Array<D, T, DeviceType> &original_data,
    ...
    while (!done_quantization) {
      ...
      if (lossless.huffman.workspace.outlier_count <=
        ...
        //std::cout << "huffman for quantization total points " << hierarchy.total_num_elems() <<" quantized_data.totalNumElems:  " << quantized_data.totalNumElems() << " outlier counts " << lossless.huffman.workspace.outlier_count << " dict size " << config.huff_dict_size << std::endl;
        if (log::level & log::TIME) {
          timer.end();
