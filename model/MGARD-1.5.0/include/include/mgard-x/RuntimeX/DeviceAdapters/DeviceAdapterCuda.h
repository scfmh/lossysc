...
MGARDX_KERL void CudaKernel(Task task, THREAD_IDX blockz_offset,
  ...
  /*Timer timer_op, timer_op1;
  timer_op.clear(); timer_op1.clear();
  float op, op1;*/
  /*cudaEvent_t opstart, opend, op1start, op1end;
  cudaEventCreate(&opstart);
  cudaEventCreate(&op1start);
  cudaEventCreate(&opend);
  cudaEventCreate(&op1end);*/
  ...
  //printf(" this is the CudaKernel details in Init task.GetGridDimZ() %d, task.GetGridDimY() %d, task.GetGridDimX() %d, task.GetBlockDimZ() %d, task.GetBlockDimY() %d, task.GetBlockDimX() %d, blockz_offset + blockIdx.z %d, blocky_offset + blockIdx.y %d, blockx_offset + blockIdx.x %d, threadIdx.z %d, threadIdx.y %d, threadIdx.x %d \n",task.GetGridDimZ(), task.GetGridDimY(),task.GetGridDimX(), task.GetBlockDimZ(),task.GetBlockDimY(), task.GetBlockDimX(),blockz_offset + blockIdx.z, blocky_offset + blockIdx.y, blockx_offset + blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x );
  //timer_op.start();
  //timer_op1.start();
  //cudaEventRecord(opstart, 0);
  //cudaEventRecord(op1start, 0);
  //long long start = clock64();
  task.GetFunctor().Operation1();
  SyncBlock<CUDA>::Sync();
  /*start = clock64() - start;
  if (threadIdx.y == 0 && threadIdx.x == 0 &&threadIdx.z == 0) 
    printf("time1: %llu\n", start); 
  start = clock64();*/
  //cudaEventRecord(op1end, 0);
  //timer_op1.end();
  ...
  //cudaEventRecord(opend, 0);
  //timer_op.end();
  //cudaEventElapsedTime(&op, opstart, opend);
  //cudaEventElapsedTime(%op1, op1start, op1end);
  //timer_op.print(task.GetFunctorName() + "_Op");
  //timer_op1.print(task.GetFunctorName() + "_Op1");
  //std::cout << task.GetFunctorName() << "_Op: " << op/1000 << " s" << std::endl;
  //std::cout << task.GetFunctorName() << "_Op1: " << op1/1000 << " s" << std::endl;
}
...
public:
  ...
  DeviceSpecification() {
    ...
    for (int d = 0; d < NumDevices; d++) {
      //std::cout<<"this is device specificication" <<NumDevices<<" "<<d <<std::endl;
  ...
  MGARDX_CONT int GetNumDevices() { std::cout<<"this is numebr of device in GetNumDevices" <<NumDevices<<std::endl; return NumDevices; }
...
public:
  ...
  MGARDX_CONT static void SortByKey(SIZE n, SubArray<1, KeyT, CUDA> in_keys,
    ...
    /*int count = 0;
    printf("this is quantization ");
    for (int i = 0; i < n; i++) {
    if (out_values.data()[i] > 0) { 
       count++; 
       printf ("%ld ", out_keys.data()[i]); }
    }
    printf("\n");
    printf( "Actual quantization is %ld \n",count);*/
    if (!workspace.hasDeviceAllocation()) {
      workspace = Array<1, Byte, CUDA>({(SIZE)temp_storage_bytes});
    }
  }
