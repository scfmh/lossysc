...
void Quantization_static(std::string name1,std::string name_f, SubArrayType subArray,SubArrayType subArray_f,  int first) {
  SIZE nrow = 1;
  SIZE ncol = 1;
  SIZE nfib = 1;
  ...
  //std::cout << "SubArray value: " << name1 << ": ";
  T *v = new T[nrow * ncol * nfib];
  T *v_f = new T[nrow * ncol * nfib];
  ...
  for (SIZE i = 0; i < nrow; i++) {
    MemoryManager<DeviceType>::CopyND(
        v_f + ncol * nfib * i, nfib,
        subArray_f.data() + subArray_f.lddv1() * subArray_f.lddv2() * i,
        subArray_f.lddv1(), nfib, ncol, 0);
  }
  DeviceRuntime<DeviceType>::SyncQueue(0);
    ...
    for (int j = 0; j < ncol; j++) {
      for (int k = 0; k < nfib; k++) {
        if (nfib * ncol * i + nfib * j + k >= first) {
             total_occur += (size_t)(v[nfib * ncol * i + nfib * j + k]) * (size_t)(v_f[nfib * ncol * i + nfib * j + k]);
            total_count += (v_f[nfib * ncol * i + nfib * j + k]);
            //std::cout << (v[nfib * ncol * i + nfib * j + k]) << " " << (v_f[nfib * ncol * i + nfib * j + k]) << " "<< total_occur << " "<<total_count << std::endl;
        } 
        }
  }
 }
 std::cout << "total occur " << total_occur << "total count" << total_count << std::endl;
  mean = static_cast<double>(total_occur) / static_cast<double>(total_count);
  for (int i = 0; i < nrow; i++) {
    for (int j = 0; j < ncol; j++) {
      for (int k = 0; k < nfib; k++) {
        if (nfib * ncol * i + nfib * j + k >= first) {
            total_dif += (double)((double)(v[nfib * ncol * i + nfib * j + k]-mean) * (double)(v[nfib * ncol * i + nfib * j + k]-mean) * (size_t)(v_f[nfib * ncol * i + nfib * j + k]));
        }
        }
  }
 }
  var = static_cast<double>(total_dif) / static_cast<double>(total_count);
...
template <typename SubArrayType>
void Quantization_mean(std::string name, SubArrayType subArray, int first) {
  SIZE nrow = 1;
  SIZE ncol = 1;
  SIZE nfib = 1;
