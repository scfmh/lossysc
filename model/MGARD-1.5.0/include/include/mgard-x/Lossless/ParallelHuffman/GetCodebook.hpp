...
void GetCodebook(int dict_size,
  ...
  /*Timer non_use_timer;
  non_use_timer.start(); 
  Quantization_mean("SortByKey::_d_qcode_subarray",_d_qcode_subarray, first_nonzero_index);
  Quantization_mean("SortByKey::_d_freq_subarray", _d_freq_subarray, first_nonzero_index);
  non_use_timer.end();
  non_use_timer.print("Non use and subtract ");
  non_use_timer.clear();*/
  #ifdef HF_STATICS
  Quantization_static("SortByKey::_d_qcode_subarray","SortByKey::_d_freq_subarray", _d_qcode_subarray,_d_freq_subarray, first_nonzero_index);
  #endif
  int nz_dict_size = dict_size - first_nonzero_index;
  std::cout << "Real quantization is  " << nz_dict_size<<  std::endl;
