...
public:
  ...
  GenTask(int queue_idx) {
    ...
    gridy = 1;
    gridx = numBlocks;
    printf("Histogram block tbz %d, tby %d, tbx %d \n",tbz, tby, tbx);
    //printf(" this is Histogram queue index %d, tbx %d, tby %d, tbz %d, gridx %d , gridy %d, gridz %d \n",queue_idx, tbx, tby, tbz, gridx, gridy, gridz);
...
MGARDX_CONT void ExecutionConfig(int N, int bins, int RPerBlock,
  ...
  numBlocks = numSMs;
  //printf("this is numSMs %d ", numSMs); 
  threadsPerBlock =
...
MGARDX_CONT void Histogram(SubArray<1, T, DeviceType> input_data,
  ...
  int maxbytes = DeviceRuntime<DeviceType>::GetMaxSharedMemorySize();
   //std::cout << "this is maxbytes " << maxbytes << std::endl;
  SubArray<1, int, DeviceType> local_histogram;
  if (bins * sizeof(int) < maxbytes) {
    ...
    DeviceLauncher<DeviceType>::Execute(
        ...
        queue_idx);
     //std::cout << "if execute N " << N << " bins " << bins << " RPerBlock " << RPerBlock << " threadsPerBlock " << threadsPerBlock << " numBlocks " << numBlocks << std::endl;
  } else {
    if (DeviceRuntime<DeviceType>::PrintKernelConfig) {
      std::cout << log::log_info
    ...
    DeviceLauncher<DeviceType>::Execute(
        HistogramKernel<T, Q, false, DeviceType>(input_data, local_histogram,
                                                 ...
                                                 threadsPerBlock, numBlocks),
        queue_idx);
    //std::cout << "else execute N " << N << " bins " << bins << " RPerBlock " << RPerBlock << " threadsPerBlock " << threadsPerBlock << " numBlocks " << numBlocks << std::endl;
