...
public:
  ...
  MGARDX_EXEC void Operation1() {
    ...
    //printf("this is GpkReo3DFunctor Operation1 \n");
    //printf(" this is R C F, %d %d %d \n", R, C, F); 
    ...
    f_gl = f + f_sm;
    f_gl_ex = f + (F / 2) * 2;
    //printf ("this is basic information  FunctorBase<DeviceType>::GetBlockIdZ() %d, FunctorBase<DeviceType>::GetBlockDimZ() %d, FunctorBase<DeviceType>::GetThreadIdZ() %d \n", FunctorBase<DeviceType>::GetBlockIdZ(), FunctorBase<DeviceType>::GetBlockDimZ(), FunctorBase<DeviceType>::GetThreadIdZ());
    
    //printf( " this is r information r %d nr %d r_sm %d R %d r_gl %d \n", r, nr, r_sm, R, r_gl);
    //printf( " this is c information c %d nc %d c_sm %d C %d c_gl %d \n", c, nc, c_sm, C, c_gl);
    //printf( " this is f information f %d nf %d f_sm %d F %d f_gl %d \n", f, nf, f_sm, F, f_gl);    
    /* Load v */
    // loading extra rules
    ...
    // Load from dv
    //printf( " this is v information v1: %d v2: %d v3: %d r_gl: %d c_gl: %d f_gl: %d \n", v.shape(0), v.shape(1), v.shape(2), r_gl, c_gl, f_gl);
    long long int start;
  ...
  MGARDX_EXEC void Operation2() {
    ...
    //printf( "this is GpkReo3DFunctor Operation2 \n ");
    if (r_sm < rest_r && c_sm < rest_c && f_sm < rest_f) {
  ...
  MGARDX_EXEC void Operation3() {
    //printf( "this is GpkReo3DFunctor Operation3 \n");
  ...
  //  start = clock64() - start;
   ...
   // blockIdx.y, blockIdx.x, start); start = clock64();
    ...
    if (!wcf.isNull() && threadId >= (R / 2) * (C / 2) * (F / 2) * 4 &&
     ...
     //asm volatile("membar.cta;");
     //long long int start = clock64() - start;
     //printf("[CF-store] block id %d,%d,%d elapsed %lu\n", blockIdx.z,
     //blockIdx.y, blockIdx.x, start);
...
public:
  ...
  GenTask(int queue_idx) {
    ...
    printf("Gpk block tbz %d, tby %d, tbx %d \n", tbz, tby, tbx);
    //printf(" this is Gpk queue index %d, total_thread_z %d, total_thread_y %d, total_thread_x %d, tbx %d, tby %d, tbz %d, gridx %d , gridy %d, gridz %d \n",queue_idx, total_thread_z, total_thread_y, total_thread_x, tbx, tby, tbz, gridx, gridy, gridz);
