...
class ComposedReconstructor
  ...
  //using Interleaver = BlockedInterleaver<D, T_data, DeviceType>;
  ...
  //using Encoder = PerBitEncoder<D, T_data, T_bitplane, T_error, DeviceType>;
...
public:
  ...
  ComposedReconstructor(Hierarchy<D, T_data, DeviceType> hierarchy,
    ...
    //std::cout << "this is second reconstructor" << std::endl;
    prev_reconstructed = false;
    partial_reconsctructed_data = Array<D, T_data, DeviceType>(
        hierarchy.level_shape(hierarchy.l_target()));
  ...
  void GenerateRequest(MDRMetadata &mdr_metadata) {
   // mgard_x::Timer timer;
    Timer timer;
    timer.start();
    //std::cout << "this is the timer start" << std::endl;
    ...
    if (mdr_metadata.requested_s == std::numeric_limits<double>::infinity()) {
      ...
      for (int i = 0; i <= target_level; i++) {
        ...
        //for (int j = 0; j < mdr_metadata.level_squared_errors[i].size(); j++ )
         // std::cout <<"this is the level and index " << i << " " << j <<  "the error " << mdr_metadata.level_squared_errors[i][j] << std::endl;
        //std::cout << "this is the error \n" << collected_error << std::endl;
      ...
      //std::cout << "this is the error part bitplanes " << mdr_metadata.requested_level_num_bitplanes.size() << " and errors  " << level_errors[0][0] << std::endl;
    } else {
    ...
    //std::cout << "this is the timer end" << std::endl;
    //timer.print("Preprocessing");
    timer.end();
    timer.print("Preprocessing");
  }
  ...
  void ProgressiveReconstruct(MDRMetadata &mdr_metadata,
    ...
    std::cout << "hierarchy.l_target() details "<< hierarchy.level_shape(hierarchy.l_target())[0] << " " << hierarchy.level_shape(hierarchy.l_target())[1] <<" " <<  hierarchy.level_shape(hierarchy.l_target())[2] << hierarchy.l_target()<< std::endl;
    for (int level_idx = 0; level_idx <= curr_final_level; level_idx++) {
      ...
      //std::cout << "this is the number of bitplanes " << num_bitplanes << std::endl;
      ...
      mdr_metadata.PrintLevelSizes(); //qzl print
      for (int i = 0; i < mdr_metadata.level_error_bounds.size(); i ++)
         std::cout << "this is the MDR level error bound " << mdr_metadata.level_error_bounds[i];
      frexp(mdr_metadata.level_error_bounds[level_idx], &level_exp);
      std::cout << " this is the level_exp " << level_exp << "number of bitplanes " <<num_bitplanes << std::endl;
      std::cout << " this is the number of bitplanes " <<num_bitplanes<<" " <<(int)mdr_metadata.prev_used_level_num_bitplanes[0] << std::endl;
      //std::cout << "encode progressive details hierarchy.level_num_elems(level_idx) " << hierarchy.level_num_elems(0) <<"levels_data[0] " << (double)levels_data[0][0] << std::endl;
    ...
    std::cout << " this is the reconstruced data size " << hierarchy.level_shape(curr_final_level)[0] << " " <<hierarchy.level_shape(curr_final_level)[1] << " "<<hierarchy.level_shape(curr_final_level)[2] << std::endl;
    timer.start();
    ...
    //std::cout << "this is the reconstruct data " << reconstructed_data.data() << std::endl;
