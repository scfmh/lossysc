...
#pragma omp parallel for
  for (SIZE dev_id = 0; dev_id < adjusted_num_dev; dev_id++) {
    //std::cout<<"this is device id and adjusted id" << dev_id << " " << adjusted_num_dev << std::endl;
...
void general_compress(std::vector<SIZE> shape, T tol, T s,
  ...
  int adjusted_num_dev =
      ...
      //std::cout << "number of devices used runtime device and config number" << DeviceRuntime<DeviceType>::GetDeviceCount() << " " << config.num_dev << std::endl;
...
#pragma omp parallel for firstprivate(config)
  ...
  //log::info("Output previously pinned: " +
           // std::to_string(output_previously_pinned));
  //std::cout<<"Input previously pinned: " << std::to_string(input_previously_pinned)<<"Output previously pinned: " <<std::to_string(output_previously_pinned)<<std::endl;
  if (log::level & log::TIME) {
    timer_each.end();
