...
void generate_request(DomainDecomposer<D, T, ComposedRefactor<D, T, DeviceType>,
  ...
  for (int subdomain_id = 0; subdomain_id < domain_decomposer.num_subdomains();
    ...
    //std::cout << " this is the subdomain_id " << subdomain_id << std::endl;
...
void refactor_subdomain_series(
  ...
  if (log::level & log::TIME)
    timer_series.start();
  Array<D, T, DeviceType> device_subdomain_buffer;
  //std::cout << "this is the total sub domains " << subdomain_ids.size() << std::endl; 
...
void reconstruct_subdomain(
  ...
  //std::cout << "this is mdr metadata " << *(double*)mdr_metadata.data[0] << "refactored data " << *(double*)refactored_data.data[0] << std::endl;
  ...
  for (DIM d = 0; d < D; d++) {
    ss << hierarchy.level_shape(hierarchy.l_target(), d) << " ";
    std::cout << "hierarchy size"  << hierarchy.level_shape(hierarchy.l_target())[d] << " ";
    //device_subdomain_buffer.resize(hierarchy.level_shape(hierarchy.l_target(), d));
  }
  ...
  std::cout << " this is the shape " << device_subdomain_buffer.shape()[0]<< " " << device_subdomain_buffer.shape()[1] << " " << device_subdomain_buffer.shape()[2] << std::endl;
  std::cout << " this is the mdr_adaptive_resolution" << config.mdr_adaptive_resolution << std::endl; 
  std::cout << " this is the mdr_metadata information " << mdr_metadata.num_levels << " " <<  (int)mdr_metadata.prev_used_level_num_bitplanes[0] << " " <<(int)mdr_metadata.loaded_level_num_bitplanes[0]<< " " <<  (int)mdr_metadata.prev_used_level_num_bitplanes[1] << " " <<(int)mdr_metadata.loaded_level_num_bitplanes[1]<< " " <<  (int)mdr_metadata.prev_used_level_num_bitplanes[2] << " " <<(int)mdr_metadata.loaded_level_num_bitplanes[2] << std::endl;
  ...
  if (config.mdr_adaptive_resolution) {
    std::cout << "this is config in include/mgard-x/MDRHighLevel/MDRHighLevel.hpp" << std::endl;
    std::cout << " this is the shape " << device_subdomain_buffer.shape()[0]<< " " << device_subdomain_buffer.shape()[1] << " " << device_subdomain_buffer.shape()[2] << std::endl;
  ...
  //std::cout << "this is the value in subdomain " << *(double*)reconstructed_data.data[0] << std::endl;
...
void MDRefactor(std::vector<SIZE> shape, const void *original_data,
  ...
  //std::cout << "original file in the datarefactor" << std::endl;
  //doublePtr += offset;
   //std::cout << *(double *)original_data << " ";
  ...
  //std::cout << "this is the refactor data sub_domains  " << domain_decomposer.num_subdomains() << std::endl;
...
void MDReconstruct(std::vector<SIZE> shape,
 ...
 //std::cout << "this is the total elements" << total_num_elem;
  ...
  if (m.dstype == data_structure_type::Cartesian_Grid_Uniform) {
    //std::cout << " this is the DomainDecomposer if branch. " << std::endl;
    domain_decomposer =
    ...
    //std::cout << " this is the DomainDecomposer eles branch. " << std::endl;
    domain_decomposer =
        DomainDecomposer<D, T, ComposedReconstructor<D, T, DeviceType>,
                         DeviceType>(
            shape, adjusted_num_dev, m.domain_decomposed,
   ...
   if (!config.mdr_adaptive_resolution) {
    if (!reconstructed_data.IsInitialized()) {
      ...
      std:: cout << " this is mdr_adaptive_resolution and first reconstruction" << std::endl;
    ...
    if (!reconstructed_data.IsInitialized()) {
      // First time reconstruction
      std::cout << "2 step and domain_decomposer.num_subdomains is " << domain_decomposer.num_subdomains()  << std::endl;
      ...
      for (int subdomain_id = 0;
        ...
        for (int i = 0;
          ...
          std::cout<< " domain_decomposer.subdomain_shape(subdomain_id)[i] i is " << i << "& value is " << domain_decomposer.subdomain_shape(subdomain_id)[i] << std::endl;
          n *= domain_decomposer.subdomain_shape(subdomain_id)[i];
        }
        std::cout << "this is the total value " << n << std::endl;
...
#pragma omp parallel for firstprivate(config)
  ...
  //std::cout << "this is the high level value" << std::endl;
  //std::cout.setf(std::ios::scientific);
  //std::cout.precision(12);
  //std::cout << "this is the high level value" << std::endl;
  //std::cout << *(double*)reconstructed_data.data[0] << " ";
  if (m.dstype == data_structure_type::Cartesian_Grid_Non_Uniform) {
    for (DIM d = 0; d < D; d++)
      delete[] coords[d];
  }
...
void MDReconstruct(RefactoredMetadata &refactored_metadata,
  ...
  {
    shape[d] = (SIZE)meta.shape[d];
    //std::cout << "this is the shape. " << shape[d] ;
 }
  data_type dtype = meta.dtype;
  ...
  if (dtype == data_type::Float) {
    ...
    //std::cout << "hi double here" << std::endl;
    if (shape.size() == 1) {
