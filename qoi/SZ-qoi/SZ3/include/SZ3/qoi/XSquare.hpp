...
namespace SZ {
    ...
    public:
	    ...
	    // T eb = - fabs(data) + sqrt(data * data + tolerance);
            //T eb = 2 * (- fabs(data) + sqrt(data * data + tolerance));
	    T eb =  fabs(2 * data) + tolerance - fabs(2 * data);
	    //printf("global_eb = %.4f, eb= %.4f, tolerance= %.4f\n", (double) global_eb, eb, tolerance);
            //T eb = 2 * (- fabs(data) + sqrt(data * data) + tolerance);
        ...
        bool check_compliance(T data, T dec_data, bool verbose=false) const {
            return (2 * fabs(data - dec_data) < tolerance);
        }
