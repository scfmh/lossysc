...
namespace SZ {
    ...
    std::vector<T> compute_average(T const * data, uint32_t n1, uint32_t n2, uint32_t n3, int block_size){
        ...
        for(int i=0; i<num_block_1; i++){
		    ...
		    //std::cout << "average in function " << sum << " " << sum / n_block_elements << std::endl;
    ...
    template<class T>
    void evaluate_average(T const * data, T const * dec_data, double value_range, uint32_t n1, uint32_t n2, uint32_t n3, int block_size = 0){
        if(block_size == 0){
	    ...
	    std::cout << " original average" << average << "decompressed average" << average_dec << std::endl;
	    ...
	    std::cout << "Sum of average.data() = " << sum_o << std::endl;
            for (const auto &value : average_dec) {
	      sum_d += value;
	     }

	    std::cout << "Sum of average.data() = " << sum_d << std::endl;
	    std::cout << std::endl;
