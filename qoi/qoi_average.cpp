...
// // template <typename T> 
float calculateAverage(const float *matrix, std::size_t rows, std::size_t cols,const std::vector<size_t>& w2d) {
  if (w2d[2] > rows || w2d[3] > cols) {
    throw std::invalid_argument("Region isn't contained in the matrix");
  }

  double total = 0.0;
  std::size_t count = 0;

  for (std::size_t i = w2d[0]; i < w2d[2]; ++i) {
    for (std::size_t j = w2d[1]; j < w2d[3]; ++j) {
      total += matrix[cols * i + j];
      ++count;
    }
  }

  if (count == 0) {
    throw std::invalid_argument("Region has zero size");
  }
  // std::cout << w2d[0] << " " << w2d[1] << " " << w2d[2] << " " << w2d[3] << " total " << total << "count " << count << std::endl;
  return total / count;
}

float calculateAverage3D(const float *matrix, std::size_t depth, std::size_t rows, std::size_t cols, const std::vector<size_t>& w2d) {

    if (w2d[3] > depth || w2d[4] > rows || w2d[5] > cols) {
        throw std::invalid_argument("Region isn't contained in the matrix");
    }
    ...
    for (std::size_t k = w2d[0]; k < w2d[3]; ++k) {
        for (std::size_t i = w2d[1]; i < w2d[4]; ++i) {
            for (std::size_t j = w2d[2]; j < w2d[5]; ++j) {
                total += matrix[(k * rows + i) * cols + j];
                ++count;
            }
        }
    }
...
Metrics calculateMetrics(const float* original, const float* decompressed, size_t size) {
    float relaError = 0.0;
    float rmseSum = 0.0;
    float maxError = 0.0;
...
result launch_compress(mgard_x::DIM D, enum mgard_x::data_type dtype,
  ...
  std::cout << "average using original data: " << calculateAverage3D(original_data, shape[2], shape[1], shape[0], w2d)
            << std::endl;
    std::cout << "average using decompressed data: " <<
            calculateAverage3D((float*)decompressed_data, shape[2], shape[1], shape[0], w2d) << std::endl;
  float err =
      std::abs(calculateAverage3D((float*)decompressed_data, shape[2], shape[1], shape[0], w2d) - 
      calculateAverage3D(original_data, shape[2], shape[1], shape[0], w2d));
  std::cout << "real error of QoI (average) = " << err << "\n";
  if (err < tol)
    std::cout << "********** Successful **********\n";
  else
    std::cout << "********** Fail with error preservation **********\n";
  Metrics metrics = calculateMetrics(original_data, (float*)decompressed_data, original_size);
  ...
  print_statistics(s, mode, shape, original_data, (float *)decompressed_data,
    ...
    std::cout << "size " << w2d.size() << std::endl;
    std::cout << "request error bound on QoI (average) = " << tol << "\n";
  std::cout << "average using original data: " << calculateAverage(original_data, shape[1], shape[0], w2d)
            << std::endl;
    std::cout << "average using decompressed data: " <<
            calculateAverage((float*)decompressed_data, shape[1], shape[0], w2d) << std::endl;
  float err =
      std::abs(calculateAverage((float*)decompressed_data, shape[1], shape[0], w2d) - 
      calculateAverage(original_data,shape[1], shape[0], w2d));
  std::cout << "real error of QoI (average) = " << err << "\n";
  if (err < tol)
    std::cout << "********** Successful **********\n";
  else
...
bool run(int argc, char *argv[]) {
  ...
  int rec = get_arg_int(argc, argv, "-q");
  std::vector<size_t> w2d;
  if (rec == 0){
    w2d.resize(4);
    w2d = {0, 0, shape[1], shape[0]};
    if (rank == 0) {
    std::string area_string = "area (";
    for (size_t index = 0; index < w2d.size(); index++)
      area_string = area_string + std::to_string(w2d[index]) + " ";
    area_string = area_string + ")";
  }
  }else if (rec == 4){
    w2d.resize(4);
    w2d = get_arg_dims(argc, argv, "-q");
    if (rank == 0) {
    std::string area_string = "area (";
    for (size_t index = 0; index < w2d.size(); index++)
      area_string = area_string + std::to_string(w2d[index]) + " ";
  ...
  }else if (rank == 0) print_usage_message("wrong qoi area.");
  ...
  // for (double tol : ebs) {
    ...
    if (dtype == mgard_x::data_type::Double) {
      res_comp_w_prefetch = launch_compress<double>(
        ...
        max_accumulate_data, compute_delay, prefetch, max_memory_footprint, w2d);
