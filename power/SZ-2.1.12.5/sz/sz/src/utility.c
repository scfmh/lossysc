...
void Timer_time_start(struct Timer *timer) {
	    clock_gettime(CLOCK_REALTIME, &timer->start_time);
}

void Timer_time_end(struct Timer *timer) {
	    clock_gettime(CLOCK_REALTIME, &timer->end_time);
	        timer->total_time += (double)(timer->end_time.tv_sec - timer->start_time.tv_sec) +
			                         (double)(timer->end_time.tv_nsec - timer->start_time.tv_nsec) / 1000000000.0;
}

double Timer_time_get(struct Timer *timer , const char *s) {
	    double time = (double)(timer->end_time.tv_sec - timer->start_time.tv_sec) +
		                      (double)(timer->end_time.tv_nsec - timer->start_time.tv_nsec) / 1000000000.0;
	    printf("%s time: %f s\n", s, time);

	        Timer_time_clear(timer);
		    return time;
}

void Timer_time_clear(struct Timer *timer) {
	    timer->total_time = 0;
}

/*void Timer_time_print(struct Timer *timer, const char *s) {
	    printf("%s time: %f s\n", s, timer->total_time);
	        Timer_time_clear(timer);
}*/
///////////////qzl energy cpp
...
/*int start_measure_cpu(){
  ...
  if ((pid = fork()) < 0) {
    ...
    if (execvp("cpu-energy-meter", cmd) < 0) {
...
void stop_measure_cpu(int pid) {
  ...
  char kill_cmd[100];
  snprintf(kill_cmd, sizeof(kill_cmd), "kill -2 %s", pid_str);
  printf("executing: %s\n", kill_cmd);

  char* result = exec(kill_cmd);
  if (result != NULL) {
    printf("Kill Output:\n%s\n", result);
    free(result);
  }
...
int start_measure_cpu() {
  pid_t pid;
  char* cmd[5]; // Increase the size of cmd array to accommodate the command and arguments.
  
  cmd[0] = "perf";
  cmd[1] = "stat";
  cmd[2] = "-e";
  cmd[3] = "power/energy-cores/"; // Specify the event want to measure.
  cmd[4] = NULL;

  /*char* cmd[8]; // Increase the size of cmd array to accommodate the command and arguments.
  cmd[0] = "perf";
  cmd[1] = "stat";
  cmd[2] = "-I";          // Specify the interval option
  cmd[3] = "200";         // Set the interval to 0.1 seconds
  cmd[4] = "-e";
  cmd[5] = "power/energy-cores/"; // Specify the event  want to measure.
  cmd[6] = "--";
  cmd[7] = NULL;*/
