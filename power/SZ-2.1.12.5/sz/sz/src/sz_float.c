...
unsigned char * SZ_compress_float_3D_MDQ_nonblocked_with_blocked_regression(float *oriData, size_t r1, size_t r2, size_t r3, float realPrecision, size_t * comp_size){
	...
	if(confparams_cpr->szMode == SZ_TEMPORAL_COMPRESSION)
		decData = (float*)(multisteps->hist_data);
#endif
        // this is zhenlu meature the power and function time/////
	struct Timer timer_test;
	double elapsed_time;
	int pid;
	pid = start_measure_cpu();
	Timer_init(&timer_test);
	Timer_time_start(&timer_test);
	//// inilization stop here ////////
	float recip_realPrecision = 1/realPrecision;
	...
	/******************************/
	Timer_time_end(&timer_test);
	elapsed_time = Timer_time_get(&timer_test,"Preparesion");
	//Timer_time_print(&timer_test,"Preparesion");
	stop_measure_cpu(pid);
        pid = start_measure_cpu();
	Timer_time_clear(&timer_test);
	Timer_init(&timer_test);
        Timer_time_start(&timer_test);
	/*******************************/
	float tmp_realPrecision = realPrecision;
	...
	free(prediction_buffer_1);
	free(prediction_buffer_2);
        printf (" this is prediction \n");
	/******************************/
	Timer_time_end(&timer_test);
	elapsed_time = Timer_time_get(&timer_test,"Prediction");
	stop_measure_cpu(pid);
	Timer_time_clear(&timer_test);
	pid = start_measure_cpu();
	Timer_init(&timer_test);
	...
	/******************************/
	Timer_time_end(&timer_test);
	elapsed_time = Timer_time_get(&timer_test,"Huffman");
	stop_measure_cpu(pid);
	Timer_time_clear(&timer_test);
	printf(" tttt %f \n", timer_test.total_time);
	pid = start_measure_cpu();
	Timer_init(&timer_test);
	Timer_time_start(&timer_test);
	/*******************************/
	...
	/******************************/
	Timer_time_end(&timer_test);
	elapsed_time = Timer_time_get(&timer_test,"Outlier");
	stop_measure_cpu(pid);
	Timer_time_clear(&timer_test);
	printf(" tttt %f \n", timer_test.total_time);
	pid = start_measure_cpu();
