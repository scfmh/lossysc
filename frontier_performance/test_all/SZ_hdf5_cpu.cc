...
result launch_compress(int DIM, const char *input_file, const char *output_file, std::vector<size_t> shape, 
 int num_sim_iterations,bool use_compression, int compute_delay, int dtype, const char * cfgFile){
  int rank, size;
  #ifdef HDF5_USE_MPI
 ...
 /**********************read information *************/
  ...
  if (rank == 0) 
  {
   std::cout << "Data per rank: " << (double)original_size*sizeof(T)/1e9 << " GB\n";
  }
...
/********************************************************************/
 ...
 status = SZ_Init(cfgFile);
  ...
  if (rank == 0) 
  {
    std::cout << "Data per rank: " << (float)original_size*sizeof(T)/1e9 << " GB\n";
  }
  ...
  H5Pclose(plistId);
    ...
    hid_t h5_file;
    std::string out_fin;
    std::string prefix = "/lustre/orion/proj-shared/csc303/zq53/mgard_new/adios_more/hdf5_all/build/"; //this is used for the write file path 
    Timer_t timer, timer_total;
    #ifdef HDF5_USE_MPI
    ...
    for (int sim_iter = 0; sim_iter < num_sim_iterations; sim_iter++) 
    {
      ...
      timer.start();
      if (DIM == 1) {
      compressed_data = SZ_compress(dtype, original_data, &compressed_size, 0, 0, 0, 0, shape[0]);
    } else if (DIM == 2) {
      compressed_data = SZ_compress(dtype, original_data, &compressed_size, 0, 0, 0, shape[1], shape[0]);
    } else if (DIM == 3) {
      compressed_data = SZ_compress(dtype, original_data, &compressed_size, 0, 0, shape[2], shape[1], shape[0]);
    } else if (DIM == 4) {
      compressed_data = SZ_compress(dtype, original_data, &compressed_size, 0, shape[3], shape[2], shape[1], shape[0]);
    } else {
    ...
    std::string aa = outputPath.substr(outputPath.rfind("/") + 1);
    out_fin = prefix + std::string(aa) + "_" + reducedPath;
    ...
    hid_t dataset = H5Dopen2(file_id, (var_name).c_str(), H5P_DEFAULT);
    ...
    H5Drefresh(dataset);
    H5Dread(dataset, H5T_NATIVE_UCHAR, mspace, fspace, H5P_DEFAULT, var_data1);
    ...
    timer.clear();
      if (use_compression) {
        ...
        if (DIM == 1) {
          SZ_decompress(dtype, compressed_data,compressed_size, 0, 0, 0, 0, shape[0]);
        } else if (DIM == 2) {
          SZ_decompress(dtype, compressed_data,compressed_size, 0, 0, 0, shape[1], shape[0]);
        } else if (DIM == 3) {
          SZ_decompress(dtype, compressed_data,compressed_size, 0, 0, shape[2], shape[1], shape[0]);
        } else if (DIM == 4) {
          SZ_decompress(dtype, compressed_data,compressed_size, 0, shape[3], shape[2], shape[1], shape[0]);
        } else {
          std::cout << "wron demisions\n";
          exit(-1);
        }
...
bool run(int argc, char *argv[]) {
 // int rank, size;
  ...
  res_comp_wo = launch_compress<float>(
        DIM, input_file.c_str(), output_file.c_str(), shape, 1, use_compression, compute_delay, dtype, cfgFile.c_str()); }
  else if (dtype == 1){
    res_comp_wo = launch_compress<double>(
        DIM, input_file.c_str(), output_file.c_str(), shape, 1, use_compression, compute_delay, dtype, cfgFile.c_str());
  }
  ...
  result res_comp;
  if (rank == 0)
    std::cout << " The configure file " << cfgFile << std::endl;
