...
double accuracy,int num_sim_iterations,bool use_compression, int compute_delay, zfp_type type, int fixed_rate, enum device_type dev_type){
 ...
 /**********************read information *************/
  ...
  //T * o_data = (T *) new T[original_size]; 
  if (rank == 0) 
  {
  ...
  hid_t h5_file;
  std::string out_fin;
  std::string prefix = "/gpfs/alpine/proj-shared/csc143/zq53/mgard_new/adios_more/IO_test/hdf5/build/"; //this is used for the write file pat
...
// Get the current time before the operation
  ...
  for (int sim_iter = 0; sim_iter < num_sim_iterations; sim_iter++) 
  {
    ...
    if (dev_type == 1){
    cudaMalloc((void**)&d_original_data, sizeof(T) * original_size);
    ...
    if (use_compression) /***use compression or not ***********/
    {
     ...
     // cudaMemcpy(d_original_data, original_data, sizeof(T) * original_size,
          //cudaMemcpyHostToDevice);
      cudaMalloc((void**)&d_compressed_data, sizeof(T) * original_size);
      ...
      bufsize = zfp_stream_maximum_size(zfp, field);
      stream = stream_open(d_compressed_data, sizeof(T) * original_size);
      zfp_stream_set_bit_stream(zfp, stream);
      zfp_stream_rewind(zfp);
    ...
    //else{ cudaMemcpy(o_data, d_original_data, original_size, cudaMemcpyDeviceToHost);
      //cudaDeviceSynchronize();}
    // ********* Write********** //
   /* if(!use_compression)
    {
    T * o_data = (T *) new T[original_size];
    cudaMemcpy(o_data, d_original_data, original_size, cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();}*/

    var_size = use_compression ? compressed_size : original_size * sizeof(T);
    if (!use_compression && (dev_type == 1))
    {
    T * o_data = (T *) new T[original_size];
    cudaMemcpy(o_data, d_original_data, original_size*sizeof(T), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    var_data = use_compression ? (unsigned char *)compressed_data : (unsigned char *) o_data;
    }
    else if (use_compression && (dev_type == 1))
    var_data =  (unsigned char *)compressed_data;
    else
   ...
   // H5Pset_fill_time(create_plist, H5D_FILL_TIME_ALLOC);
    ...
    timer.clear();
    timer_total.end();
    double write_time_all = timer_total.get();
     #ifdef HDF5_USE_MPI
    double max_write_time_all;
    MPI_Reduce(&write_time_all, &max_write_time_all, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    if (rank == 0)
    {
      res.write = max_write_time_all;
      std::cout << "Write time: "<< max_write_time_all << "\n";
    }
    #else
    if (rank == 0)
    {
      res.write = write_time_all;
      std::cout << "Write time: "<< write_time_all << "\n";
    }
    #endif
    timer_total.clear();
  }
  ...
  for (int sim_iter = 0; sim_iter < num_sim_iterations; sim_iter++) 
  {
    ...
    H5Drefresh(dataset);
    H5Dread(dataset, H5T_NATIVE_UCHAR, mspace, fspace, H5P_DEFAULT, var_data);
    ...
    if (use_compression) {
      ...
      if (dev_type ==0){
        ...
        cudaMalloc((void**)&d_compressed_data, compressed_size);
      cudaDeviceSynchronize();
       if (D == 1) {
        field = zfp_field_1d(decompressed_data, type, shape[0]);
        } else if (D == 2) {
