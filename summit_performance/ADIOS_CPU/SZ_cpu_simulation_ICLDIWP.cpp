...
/** ADIOS class factory of IO class objects */
 ...
 /**********************read information *************/
 readfile(input_file.c_str(), original_data, original_size * sizeof(float));
 ...
 for (int sim_iter = 0; sim_iter < num_sim_iterations; sim_iter++) 
 {
  ...
  // ********* Compression ********** //
   ...
   compressed_data = SZ_compress(SZ_FLOAT, original_data, &compressed_size, 0, 0, shape[2], shape[1], shape[0]);
   #if MYADIOS_USE_MPI
   MPI_Barrier(MPI_COMM_WORLD);
   #endif
   timer.end();
   double compression_time = timer.get();
   //std::cout << "rank " << rank << " compression size: " << compressed_size << "\n";
   #if MYADIOS_USE_MPI
 ...
 for (int sim_iter = 0; sim_iter < num_sim_iterations; sim_iter++) 
 {
  ...
  if (1) {
    ...
    SZ_decompress(SZ_FLOAT, compressed_data,compressed_size, 0, 0, shape[2], shape[1], shape[0]); 
