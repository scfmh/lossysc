...
result  launch_compress(int D, const char *input_file, const char *output_file, std::vector<size_t> shape, 
 ...
 int rank, size;
Timer pointer, timer_total, app_all;
app_all.start();
pointer.start();
#if MYADIOS_USE_MPI
...
/*this will list the general zfp information*/
 ...
 unsigned char * compressed_data = (unsigned char *) new float[original_size];
 compressed_size = original_size * sizeof(float);
 float * decompressed_data = (float *) new float[original_size];
 ...
 pointer.end();
double temp = pointer.get();
 if (rank == 0)
 {
  std::cout << "Start to allocation 1: "<< temp << "\n";
 }
 pointer.clear();
 Timer timer; 
/* #if MYADIOS_USE_MPI
 ...
 //{
  ...
  sleep(compute_delay);
   pointer.start();
   cudaMalloc((void**)&d_original_data, sizeof(float) * original_size);
   cudaMemcpy(d_original_data, original_data, sizeof(float) * original_size,
 ...
 if (rank == 0)
 {
  std::cout << "load original data 2: "<< temp << "\n";
 }
 ...
 //timer_total.start();
 //cudaMalloc((void**)&d_original_data, sizeof(float) * original_size);
 //cudaMemcpy(d_original_data, original_data, sizeof(float) * original_size, cudaMemcpyHostToDevice);
 timer_total.start(); 
 if (use_compression) /***use compression or not ***********/
  ...
  // ********* Compression ********** //
   ...
   //cudaMemcpy(d_original_data, original_data, sizeof(float) * original_size,
           // cudaMemcpyHostToDevice);
   cudaMalloc((void**)&d_compressed_data, sizeof(float) * original_size);
   cudaDeviceSynchronize();
   if (D == 1) {
   ...
   stream = stream_open(d_compressed_data, sizeof(float) * original_size);
 ...
 timer.start();
 adios2::Engine writer = io.Open(output_file, adios2::Mode::Write);
 //simulation_var.SetMemorySpace(adios2::MemorySpace::CUDA);
 writer.BeginStep();
 writer.Put<unsigned char>(simulation_var, var_data, adios2::Mode::Deferred);
 writer.PerformPuts();
 writer.EndStep();
 //writer.Close();
 cudaDeviceSynchronize();
...
if (rank == 0)
 ...
 //{
  ...
  if (use_compression) {
    ...
    unsigned char *d_compressed_data;
    cudaMalloc((void**)&d_decompressed_data, sizeof(float) * original_size);
   ...
   cudaDeviceSynchronize();
    ...
    cudaDeviceSynchronize();
    cudaMemcpy(decompressed_data, d_decompressed_data, sizeof(float) * original_size,
...
/*    #if MYADIOS_USE_MPI
 ...
 if (rank == 0)
 {
  std::cout << "Total read time: " << temp << "\n";
 }

 app_all.end();
 double a_time = app_all.get();
 if (rank == 0) 
 {
  std::cout << "Total read time: " << a_time << "\n"; 
 }
...
std::string dt = get_arg(argc, argv, "-t");
 if (dt.compare("s") == 0) {
  ...
  //std::vector<double> ebs = {16.7};
  std::vector<int> ebs = {4};
  //std::vector<int> ebs = {4, 7, 12, 15};
  result res_no_comp;
...
int rank;
 ...
 size = 1;
  ...
  run(argc, argv);
 /* app.end();
  double time_app = app.get();
  std::cout << "all app time " << time_app << std::endl;*/
  #if MYADIOS_USE_MPI
  MPI_Finalize();
  #endif
  app.end();
